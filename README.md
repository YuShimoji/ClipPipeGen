...
次の自動化アンカーは実 downloader 接続（INT-02 successor）だが、先に INT-02b として [asset_fetch 境界仕様](docs/ASSET_FETCH_BOUNDARY.md) を固定済み。INT-02c では URL / VOD / network fetch へ進まず、既存ローカル media file を FFmpeg で `source.wav`（PCM WAV / mono / 16kHz / 16-bit）に正規化する adapter だけを `src/integrations/asset_fetch/` に追加した。INT-02d では [yt-dlp-audio 境界仕様](docs/YTDLP_AUDIO_SPEC.md) を spec only として固定済み。INT-02e では `fetch-source-audio --mode yt-dlp-audio` を source audio URL fetch のみに限定して実装し、実 URL smoke で receipt / sidecar / ledger / WAV readback まで確認した。SH-05 では、ローカル media 1本から `source.wav`、fake/fixture transcript、cut candidates、context status、subtitle draft、`preview_manifest.json`、read-only `preview_report.html` までを 1 command で生成する review surface を追加した。SH-05d では既存 source audio material の `source.wav` / receipt / sidecar / ledger を `preview_manifest.json` と `preview_report.html` に接続し、source audio provenance を operator が読めるようにした。ED-06 では `edit_pack.json` から `nle_cut_list.csv` / `nle_export_manifest.json` / `nle_export_report.html` を生成し、cut range・subtitle draft・source refs を external editor へ渡せる最小出口を追加した。GUI から生成・fetch・render は実行しない。`transcribe-audio` は既存のローカル音声ファイルを `transcript.json` にする責務に限定し、URL / VOD からの取得は INT-02 として分離する。`fetch-source-video` / GUI fetch button / render / encode / FCPXML・Resolve XML はまだ未実装。
