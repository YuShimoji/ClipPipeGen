...
| review_status | Human response consumed as `adjust_boundary`; diagnostic / representative only. |
| next_action | Use `clip-typography-decoration-comparison-001` to choose the next font-family / decoration direction. |
...
| generated_from | `build-subtitle-typography-decoration-comparison` reading existing ignored episode `edit_pack.json` for `cut_002` / `cut_003` review text. |
| validation_command | `uvx --with pillow pytest -q tests/test_subtitle_style_spike.py` plus normal project validation. |
| latest_validation_result | `6 passed` for the Pillow-enabled style-spike path; normal no-Pillow path can skip PNG generation tests. |
| latest_local_smoke | Same-machine generation produced 4 candidates, 16 PNG samples, JSON/HTML report, contact sheet, and `open_comparison.ps1`; `font_size_policy.value=124`, `production_subtitle_design_acceptance=false`, `rights_status=pending`. |
| review_status | Pending human choice of font-family / decoration candidate for the next diagnostic overlay proof direction. |
| next_action | Ask which candidate id should drive the next diagnostic overlay proof, or whether a small candidate adjustment is needed. |
