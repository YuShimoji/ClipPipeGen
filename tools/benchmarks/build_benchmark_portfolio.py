...
<body><header><h1>ClipPipeGen benchmark portfolio</h1><p>登録済み 15 family / 27 family-scoped candidate slot を、現物に基づく最小レビュー面へ接続した内部用台帳です。媒体の重複利用は別比較 slot として数え、<code>reuse_of</code> で明示します。権利・production・公開・収益化・upload 承認は推定しません。</p><div class="counts">{count_cards}</div><p>Observed at: <code>{html.escape(portfolio['observed_at'])}</code> · <a href="benchmark_portfolio.json">JSON ledger</a> · <a href="COVERAGE_LEDGER.md">Markdown ledger</a></p></header><main>{''.join(cards)}</main></body></html>"""
