...
def test_out14_current_state_preserves_out13_receipt_without_opening_external_gates() -> None:
    ...
    for text in (runtime, handoff):
        assert "current_slice: OUT-14" in text
        assert "human_review_pending: true" in text
        assert "editorial_acceptance_granted: false" in text
        assert "acceptance_receipt: null" in text
        assert "main_integration_approved: false" in text
        assert "m4_main_integration_status: not_applicable_out14" in text
        assert "m5_integrated_baseline_verification_status: not_applicable_out14" in text
