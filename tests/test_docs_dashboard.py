...
def test_ed10bc_resume_surfaces_are_current_and_ed10ba_sources_remain_linked():
    current_out11_artifact = "clip-edit-ready-source-packet-v1-001"
    ...
    for path in [
        ...
        if path.name == "CURRENT_HANDOFF.md":
            ...
            assert (
                ...
                "source_packet_report.html" in text
            )
...
def test_artifact_registry_records_content_planning_and_ed10ah_sources():
    ...
    assert status["current_focus"]["canonical_status"] == (
        "edit_ready_source_packet_operational_v1"
    )
    assert status["current_focus"]["review_status"] == (
        "machine_validated_edit_ready_input_available"
    )
    ...
    assert status["current_focus"]["human_entrypoint"] == (
        "episodes/edit_ready_source_packet_20260724/source_packet/"
        "clip-edit-ready-source-packet-v1-001/source_packet_report.html"
    )
    ...
    assert status["current_focus"]["machine_readback"] == (
        "episodes/edit_ready_source_packet_20260724/source_packet/"
        "clip-edit-ready-source-packet-v1-001/source_packet.json"
    )
    ...
    assert status["current_focus"]["review_server_status"] == (
        "not_applicable_passive_html_readback"
    )
    assert (
    ...
    current_surfaces = [
        ...
        if item["label"] == "SH-10 Current Focus"
    ]
    assert current_surfaces == [
        {
            "label": "SH-10 Current Focus",
    ...
    assert "clip-edit-ready-source-packet-v1-001" in artifact_ids
