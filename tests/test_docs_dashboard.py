...
def test_docs_dashboard_detects_unclear_and_over_guarded_docs(tmp_path: Path):
    ...
    assert status["current_focus"]["feature_id"] == "ED-10al"
    ...
    assert status["current_focus"]["state"] == (
        "final_render_path_stage_3_diagnostic_rehearsal_ready"
    )
    assert status["current_focus"][
    ...
    assert status["current_focus"]["route_status"] == (
        "ed10al_final_render_path_stage_3_rehearsal_ready_active_ed10af_l2_preserved"
    )
    ...
    assert status["current_focus"]["review_surface_direction"] == (
        "final_render_path_stage_3_rehearsal_no_review_card"
    )
    ...
    assert status["current_focus"]["review_memory"]["current_blocker"] == (
        "none_for_final_render_path_stage_3_rehearsal"
    )
    ...
    assert status["current_focus"]["review_card"]["axis"] == (
        "final_render_path_stage_3_diagnostic_rehearsal"
    )
    ...
    assert status["current_focus"]["review_debt"][0]["status"] == (
        "final_render_path_stage_3_rehearsal_ready_limitation_lift_next"
    )
    ...
    assert "ED-10z local readback" in persisted["open_surfaces"][15][
        "when_to_use"
    ]
...
def test_docs_dashboard_current_focus_registration_uses_active_ed10al_artifact(
    tmp_path: Path,
):
    _write_fixture_docs(tmp_path)
