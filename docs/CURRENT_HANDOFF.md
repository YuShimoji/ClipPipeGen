...
The durable OUT-06 H0 repair state is a tracked builder/CLI/test/contract plus
one ignored same-machine package at
`episodes/jp_pilot01_hololive_bancho_20260525/review/out06_complete_narrative_short_delivery_candidate/`.
Its `open_preview.ps1` is the single entrypoint, with `-Serve` as the primary
localhost path when seekability matters. `serve_preview.ps1` now uses
`src.cli.serve_review`, a fixed-root 127.0.0.1 byte-range server. The page plays
one 38.633333-second H.264/AAC 1080x1920 candidate first and keeps timeline,
frames, subtitles, probe/provenance, and gates folded.
...
wraps and review-page seeking. The six repaired display wraps are `sub_013`
`なんで / 来なかった / んすか！！`, `sub_014` `ずっと / 待ってたんすよ！！`,
`sub_019` `はじめの勝ちって / ことでいいですね？`, `sub_024`
`団長、ちなみに、 / 他の番長 / 知ってますか？`, `sub_028`
`マリンなら / あっちにいたよ`, and `sub_029` `ありがとう / ございますー！`.
Only those six display line breaks changed.

The final repaired MP4 SHA-256 is
`02cfc1b25afbc7b280481453cb53c8f66d915a39389098cb70e2f37b31504bf0`.
Full decode, both PCM boundary checks, 16-point frame QA, manifest hashes, and
browser seek QA passed. Audio measured `-19.21 LUFS / -2.11 dBTP` before
normalization and `-14.39 LUFS / -1.49 dBTP` in the final output. Range QA
returned `206 Partial Content` for `bytes=100-149`, with `Accept-Ranges: bytes`,
`Content-Range: bytes 100-149/20780180`, and `Content-Length: 50`. Browser
seek QA reached 25%, 60%, and 90% with `seeked`, max target delta about
`0.0008s`, `playStatus=resumed`, readyState `4`, no media error, no horizontal
overflow, and all details folded. Native pointer-drag proof is not claimed.
...
No broad user review is pending for this H0 repair. The supervisor can close the
repair evidence or explicitly open H1. Do not infer overall human acceptance,
main merge, production/public use, thumbnail, metadata, publishing, or rights
approval from this repair.
...
page plays one H.264/AAC 1080x1920 candidate first, then shows the three bounded
review questions and collapsed reframe, frame, subtitle, probe, provenance, and
gate readback.

The candidate preserves accepted OUT-04 `cut_001 -> cut_002`, the `6.840s`
hard cut, and all nine subtitle events. Full-fit over a source-derived canvas
was selected after three-way still comparison. The final MP4 is `11.700s`,
30fps, yuv420p, faststart, and has SHA-256
`d2a75ed5f85a0869d4178917c258624ccf083bbefce33ab468549f93a982b827`.
Full decode and browser/media/layout validation passed. Audio moved from
`-19.22 LUFS / -2.11 dBTP` to `-14.06 LUFS / -1.49 dBTP` because the source
sequence had a clear loudness gap for this candidate.

The user accepted this internal OUT-05 milestone on 2026-07-12 JST. Vertical
framing and subject/action preservation passed, subtitle position/wrap/readability
passed, and audio/boundary/screen export integrity passed. The acceptance scope
is OUT-05 only; no rights, production, production subtitle-design, publishing,
or public-use gate changed.
...
| Lane | Source branch | Head used | Preserved artifact |
|---|---|---:|---|
| TRI-01 | `codex/tri-01-safety-overcapture-triage-v0` | `b04e422` | `clip-tri01-safety-overcapture-triage-v0-001` |
| HUB-01 | `codex/hub-01-external-source-registry-v0` | `4f780d5` | `clip-hub01-external-source-registry-v0-001` |
| OUT-01 | `codex/out-01-output-layer-gap-logger-v0` | `e903120` | `clip-out01-output-layer-gap-logger-v0-001` |
| EWS-05 | `codex/ews-05-human-ok-fetch-prep-ready-v0` | `d2adc66` | `clip-ews05-human-ok-fetch-prep-ready-package-v0-001` |

For the EWS-05 package, the repeatable local commands remain:
...
explicit ignored local skeletons, local source identity decision records,
docs/readme pointers, and targeted tests as allowed local actions. Source URL
opening, fetch/download, transcript, render, thumbnail proof, and local media
processing are deferred local actions. Public upload/publication, OAuth/API
keys/credentials, payment, rights/legal approval claims, destructive git,
cross-repo edits, and irreversible source overwrite remain true external gates.

Do not treat EWS-05 as fetch approval, production/public/rights/publishing/
monetization approval, transcript/render creation, thumbnail creation, or media
initialization. The CLI commands require an explicit workspace path and local
decision JSON only. Keep tracked `episodes/` empty. CPD-12 remains available at
`docs/content_planning/operator_cockpit.html`, but this handoff has already
recorded the supplied human OK report for the current candidate. The next gate
is `explicit_private_fetch_smoke_approval_required`.
...
worker did not open the source URL.
...
fetch-prep planner. It reads the workspace manifest, EWS-02 inspection path,
automation contract, and EWS-03 `source_identity.decision.json`, then emits a
parseable plan to stdout and writes `source_fetch_prep_plan.json` in the
...
The planner names future source receipt and material ledger paths but creates
no media or receipts. Missing decisions block with
`source_identity_decision_missing`; `pending`, `ng`, and `hold` block with
their corresponding source identity reasons. `ok` with `allows_fetch_prep=true`
returns `prep_state=ready_for_future_private_fetch_plan` while preserving
`fetch_authorized=false`, `media_downloaded=false`, `rights_approved=false`,
and `public_ready=false`.

## Current EWS-03 Source Identity Decision Intake / Local Decision Record v0

EWS-03 checkpoint, 2026-07-07 JST:
`clip-ews03-source-identity-decision-intake-v0-001` is the active tracked local
decision intake. The prepare command writes
`source_identity_decision.template.json` with `identity_decision=pending`. The
record command writes `source_identity.decision.json` after validating
`pending`, `ok`, `ng`, or `hold`; unknown values are rejected, and `ok` requires
reviewer or notes.
...
`ok` may set `allows_fetch_prep=true` for a later planning slice, but
`fetch_authorized=false`, `rights_approved=false`, and `public_ready=false`
remain fixed in the record.

## Current EWS-02 Episode Workspace Inspector / Manifest Consumer v0

EWS-02 checkpoint, 2026-07-07 JST:
`clip-ews02-episode-workspace-inspector-v0-001` is the active tracked local
workspace consumer. It reads a skeleton created by EWS-01 and emits readiness
JSON. A complete tempdir skeleton reports `manifest_state=initialized`,
`source_identity_state=pending`, `readiness_level=source_identity_pending`,
`skeleton_ready=true`, `ready_for_source_identity_decision=true`, and
...
The inspector does not create files unless an operator explicitly supplies
`--output`, and that output is only a JSON status report. It does not open
source URLs, fetch/download media, create transcripts, edit packs, renders,
thumbnails, uploads, credentials, rights decisions, or public-ready claims.
...
CPD-12 checkpoint, 2026-07-07 JST:
`clip-cpd12-minimal-review-console-v0-001` is the active tracked
...
plans, and CPD-05 source inspection packet records into one minimal Review
Console. The JSON exposes `shell`, `modes`, `current_work_item`,
`provenance_badges`, `status_rail`, `gate_summary`, `link_targets`, and
`candidate_groups` so the page can distinguish fixed UI shell from dynamic
candidate data.
...
Expected readback: `ux.version=v7_minimal_review_console_v0`,
`shell.default_mode=review`, `mode_count=3`, `status_rail_chip_count=4`,
`provenance_badge_count=4`, `link_target_count>=10`,
`candidate_ledger_row_count=5`, `ledger_layout=responsive_ledger_stacked`,
...
1. Open the cockpit and use Review mode for the one current planning-label
...
`clip-ed10ay-thank-ed10au-local-access-recovery-readback-001` records that the
Thank terminal had ED-10ax and the ED-10au launcher, but lacked the ignored
ED-10au MP4/ASS/local manifest at slice start. The absence was current-host
local media absence, not a Git tracked-file problem, and the user was not asked
to search again.

The required source video/audio plus transcript and edit pack were present, so
the same ED-10au local artifact path was regenerated through the existing
bounded builder. Final access is `verified_present`; the MP4 is present at
...
with size `3443682` bytes and ffprobe readback `9.18s`, H.264/AAC,
1920x1080, 30fps. The ASS and local manifest are present too.

This is a host access recovery readback only. It does not replace ED-10ax,
does not create representative-micro-scene-v2, screenshot capture, stage-7
normalizer, tracked media, or production/public/rights/publishing/
...
handoff artifact. It consumes ED-10aw and ED-10av, then makes the ED-10au
micro-scene specimen reviewable by stating what a later reviewer should judge,
what the surface is not asking, how to interpret the user's observation, and
which route to use next.

Do not ask the user now. The later review frame is freeform, not a fixed form
or yes/no table, and has exactly three look-for points: whether the specimen
purpose makes sense, which axis is the main issue, and what one next axis would
reduce review friction most.

ED-10ax creates no render, media, screenshot, representative v2 specimen,
stage-7 normalizer, or tracked `episodes/` artifact. It grants no production,
public, rights, publishing, monetization, micro-scene, or subtitle design
approval. Use layout screenshot capture, representative v2, or final
render-path stage-4 only under the explicit conditions recorded in the
artifact.
...
is the source tracked handoff artifact for ED-10ax. It consumes ED-10av, classifies the
...
The source review-frame plan is
`clip-ed10aw-grill-me-adoption-readback-and-review-frame-clarification-plan-001`.
...
Continue from
`clip-ed10ax-review-frame-clarification-surface-001` when the next terminal
needs the latest active handoff. ED-10aw remains the source review-frame plan,
ED-10av remains the source observation-frame readback, ED-10au remains the
source specimen, ED-10at remains the source observation readback, ED-10as
remains the access sheet source, and ED-10ar remains the internal review video
package source.

Treat the ED-10ax artifact as a review-frame clarification surface only:
Grill-me is still a local bounded helper, not repo policy; ED-10av observation
is classification only; no production, public, rights, publishing,
monetization, micro-scene, or subtitle design approval is inferred. Do not
request another user review now, fixed form, yes/no response, screenshot,
display polish, another Candidate 0-3 comparison, another corrected ED-10u
`cut_008` review, or general `cut_002` / `cut_003` font-family acceptance from
this handoff.
...
|---|---|---|
| `clip-ed10ax-review-frame-clarification-surface-001` | Active tracked ED-10ax review-frame clarification surface; states what ED-10au asks a later reviewer to judge, what it is not asking, and which route to use next without new media or approval | `see docs\style_intent\review-frame-clarification-surface.md` |
| `clip-ed10aw-grill-me-adoption-readback-and-review-frame-clarification-plan-001` | Source tracked ED-10aw tooling-adoption and review-frame clarification plan; keeps Grill-me bounded, forbids nested prompts, and sets review-frame clarification as first default without new media or approval | `see docs\style_intent\grill-me-adoption-readback-and-ed10aw-review-frame-clarification-plan.md` |
