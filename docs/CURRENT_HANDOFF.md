...
PID `23616` が exact `src.cli.serve_review` command で配信している。package は
...
| 確認対象 | 目的 | 結果 | 残る判断 |
|---|---|---|---|
| Git 同期 | 現行 OUT-08 branch を remote と一致させる | fetch/pull 済み。同期時点の upstream parity `0 0`、branch は `origin/main` より 2 commit 先 | この handoff/test refresh を push 後に最終 parity を再確認する |
| OUT-08 package | 同一端末証拠が tracked contract と一致するか確かめる | 16 payload hash、manifest self-integrity、2 candidates、single-column page、単一 review question が pass | candidate の編集上の自然さは機械判定しない |
...
| playback delivery | レビュー入口を再開可能にする | server を PID `23616` で復旧。page HTTP 200、MP4 Range 206、両 MP4 full decode pass | native control の direct playback/seek と編集上の違和感は人間確認 |
| 開発 health | 現行 branch から安全に実装を続けられるか確かめる | `uvx --with pillow pytest -q` は 520 passed、GUI/Electron smoke は OK | Pillow なしの plain `uvx pytest -q` は依存不足で collection error になる |

full suite が検出した 1 件は、OUT-07 時代の test が active rebuild contract を
要求していた取り残しだった。runtime の現行意味は OUT-08 が active、OUT-07 の
`artifacts/ACTIVE_REBUILD.json` は parked predecessor であるため、test をこの二点を
同時に守る regression へ修正した。artifact bytes、cut/caption authority、rights、
production/public gate は変更していない。
