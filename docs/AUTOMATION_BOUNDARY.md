...
| Local/External tool | diagnostic render proofs / editorial and vertical candidates | `src/cli/render_tiny_proof.py` / `src/cli/build_editorial_sequence.py` / `src/cli/build_vertical_short_candidate.py` / `src/cli/build_complete_narrative_short.py` / `src/integrations/render/` | OUT-01 は tiny diagnostic proof、OUT-04 は explicit ordered 2-3 cut representative sequence、OUT-05 は accepted OUT-04 timeline の 1080x1920 internal candidate、OUT-06 は accepted OUT-05 opening と authoritative cut_003 の complete narrative internal delivery candidate を生成する。atomic staging、ASS/libass、reframe/audio/provenance readback を持つが、GUI action / production render / publishing / production subtitle design acceptance は行わない |
