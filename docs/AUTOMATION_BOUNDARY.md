...
Local diagnostic processing is allowed while `rights_manifest.compliance_check.status` is `pending`: source fetch, transcript generation/import/review, cut generation, context checks, subtitle drafts, NLE CSV export, `render-tiny-proof`, bounded `build-editorial-sequence`, cut review packets, and evidence summaries. These operations must preserve rights status as readback and must not claim production, creative, publishing, or public-use approval.
...
| Local/External tool | diagnostic render proofs / editorial sequence | `src/cli/render_tiny_proof.py` / `src/cli/build_editorial_sequence.py` / `src/integrations/render/` | OUT-01 は source_video/audio + selected cut から tiny diagnostic video/receipt/manifest/report を生成し、preflight/fallback/subtitle timing readback まで実装済み。OUT-04 は explicit ordered 2-3 cuts を source-level で抽出・hard-cut し、sequence-relative 字幕付き H.264/AAC representative review MP4/readback/HTML を生成する。いずれも FFmpeg/FFprobe を render integration に閉じ込め、GUI action / production render / publishing / production subtitle design acceptance は行わない |
...
| `src/integrations/render/` | OUT-01 tiny diagnostic render と OUT-04 bounded editorial representative sequence。FFmpeg/FFprobe で source video/audio + validated cut ranges を diagnostic/review output artifact にし、subtitle timing・sequence rebasing・probe readback を保持する | URL / VOD 取得、STT、cut 候補抽出、production subtitle design、GUI action、production render acceptance |
