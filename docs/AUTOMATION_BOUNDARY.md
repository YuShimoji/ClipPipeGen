...
Local diagnostic processing is allowed while `rights_manifest.compliance_check.status` is `pending`: source fetch, transcript generation/import/review, cut generation, context checks, subtitle drafts, NLE CSV export, `render-tiny-proof`, bounded `build-editorial-sequence`, accepted-timeline `build-vertical-short-candidate`, bounded `build-complete-narrative-short`, bounded `build-real-unused-range-short-minibatch`, hash-bound endpoint preflight/evidence, bounded `build-source-adaptive-short-candidate`, combined evidence-only portfolio review, one-command internal long-form `build-real-video`, explicit caption-evidence editorial candidate `build-editorial-video-candidate`, completed-stream `build-push-microarc-stream`, exact editorial reconstruction `build-push-microarc-editorial-v2`, cut review packets, and evidence summaries. These operations must preserve rights status as readback and must not claim production, creative, publishing, or public-use approval.
...
| Local/External tool | OUT-13 caption-evidence editorial representative video | `src/cli/build_editorial_video_candidate.py` / `src/integrations/render/editorial_video_candidate.py` / `src/cli/serve_review.py` | 取得済み実sourceとhash-bound editorial plan、transcript、provider JSON3 sidecar、rights snapshotから、非連続・時系列保持のselected/omitted ranges、根拠segment、context、字幕presentation、H.264/AAC render、full validation、manifest、video-first localhost reviewをatomic packageにする。provider text/timingの取得receiptとexact hashを照合するが、公式著者性は主張しない。source-native対話captionのselected-range非存在を証拠化し、speaker未確認時はbadgeを描画しない。rights、human acceptance、production subtitle/design/render acceptance、thumbnail、public/publishing/uploadは推定・実行しない |
| Local/External tool | OUT-14 Push Micro-Arc completed-stream profile | `src/cli/build_push_microarc_stream.py` / `src/integrations/render/push_microarc_stream.py` / `src/cli/serve_review.py` | 取得済みの公開完了stream、provider info、video/audio/caption receipts、ledger、rights snapshot、exact planをhash照合し、一つの連続source rangeを通常5–15分のclosed micro-arcへする。hook/context/development/payoff/aftermathの5 role、source complement omissions、provider-auto captionと非逐語semantic evidenceの分離、creator-context namespace、metadata source-first、1080p H.264/AAC、full validation、immutable manifest、video-first reviewを作る。単一cutはinternal boundaryを捏造せずrange endpointsをevidence化する。`CATALOG_TOPIC_FEATURE` / `EVENT_STACK_RECAP`、rights、人間受領、production/YPP/thumbnail/Shorts/public/publishing/uploadは実装・推定しない |
| Local/External tool | OUT-14 Push Micro-Arc editorial reconstruction v2 | `src/cli/build_push_microarc_editorial_v2.py` / `src/integrations/render/push_microarc_editorial_v2.py` / `src/cli/serve_review.py` | 3以上のstream identityと6以上のepisode candidate、score前hard gate、competitive observationを入力authorityとして、v1 human reject/quarantineとdisjointな一episodeを選ぶ。anonymous HD source window、actual-audio ASR word timing、provider-caption provenance、multiple chronological cuts、speechとcreator telopの別namespace、actual-frame thumbnail rough、title alternatives、full media validation、video-first reviewをatomic packageへする。provider captionをviewer authorityにせず、非発話annotationとknown-v1 lociをfail-closedで除外する。recovery modeはexact failed artifact identity内のcompleted renderだけをsame-volume hardlinkでvalidation retryへ渡し、SHA/sizeを再検証する。human editorial/language/title/thumbnail acceptance、rights、production subtitle/font/render、YPP、public/publishing/upload/visibilityは実装・推定しない |
...
- production render acceptance / production subtitle burn-in-design / public-ready render surface（OUT-13でexplicit editorial plan、caption burn-in、real-video reviewまで実装済みだが、production・rights・public acceptanceへは昇格していない）
...
| `src/integrations/render/` | OUT-01 tiny diagnostic render、OUT-04 bounded editorial sequence、OUT-05/06 vertical internal candidate、OUT-07 accepted-baseline operator package、OUT-08 unused-range minibatch、OUT-09 second-source repeatability、OUT-10 third-source candidate、OUT-11 endpoint evidence / source-adaptive candidate / five-source combined review、OUT-12 one-command real long-form automation、OUT-13 explicit caption-evidence editorial candidate、OUT-14 Push Micro-Arc completed-stream profileとeditorial reconstruction v2。取得済みsource/caption/plan authority、Timeline IR、source-specific composition/caption policy、actual-audio canonical transcript、media/audio/frame/manifest/readbackを担当する | URL / VOD取得、STT実行、rights判断、human acceptance、production subtitle/design/render acceptance、GUI action、upload、thumbnail upload、visibility、made-for-kids、publishing |
