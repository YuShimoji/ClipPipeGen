...
| Local/External tool | diagnostic render proofs / editorial and vertical candidates / operator delivery pack | `src/cli/render_tiny_proof.py` / `src/cli/build_editorial_sequence.py` / `src/cli/build_vertical_short_candidate.py` / `src/cli/build_complete_narrative_short.py` / `src/cli/build_operator_delivery_pack.py` / `src/cli/reconstitute_out07_review.py` / `src/cli/serve_review.py` / `src/integrations/render/` | OUT-01 は tiny diagnostic proof、OUT-04 は explicit ordered 2-3 cut representative sequence、OUT-05 は accepted OUT-04 timeline の 1080x1920 internal candidate、OUT-06 は accepted OUT-05 opening と authoritative cut_003 の complete narrative internal delivery candidate。OUT-07 は historical accepted-only copy route を保ったまま、qualified same-source media revision から fixed semantic baseline と source-pixel poster A/B/C を再構成する separate route を持つ。current hostではedit/transcript/decision/proxy/caption authorityを直接検証し、fresh cloneではtracked semantic snapshotと既存asset-fetch material chainを使える。revision output は historical hash 一致時だけ acceptance を継承し、それ以外は human-unaccepted candidate とする。`serve_review` は固定rootの127.0.0.1 byte-range localhost review serverであり、atomic staging、ASS/libass、reframe/audio/provenance readback、thumbnail provenance、copy controls を持つが、GUI action / production render / publishing / upload / thumbnail upload / visibility / made-for-kids / production subtitle design acceptance は行わない |
...
| `src/integrations/render/` | OUT-01 tiny diagnostic render、OUT-04 bounded editorial representative sequence、OUT-05 accepted-timeline vertical internal candidate、OUT-06 complete narrative internal delivery candidate、OUT-07 internal operator delivery pack。FFmpeg/FFprobe で source video/audio + validated cut ranges を diagnostic/review output artifact にし、subtitle timing・sequence rebasing・reframe/audio/probe readback・thumbnail provenance・metadata draft gates を保持する | URL / VOD 取得、STT、cut 候補抽出、production subtitle design、GUI action、production render acceptance、upload、thumbnail upload、visibility、made-for-kids、publishing |
