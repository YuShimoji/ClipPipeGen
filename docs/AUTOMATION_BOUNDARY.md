...
| Local/External tool | diagnostic render proofs / editorial and vertical candidates | `src/cli/render_tiny_proof.py` / `src/cli/build_editorial_sequence.py` / `src/cli/build_vertical_short_candidate.py` / `src/cli/build_complete_narrative_short.py` / `src/cli/serve_review.py` / `src/integrations/render/` | OUT-01 は tiny diagnostic proof、OUT-04 は explicit ordered 2-3 cut representative sequence、OUT-05 は accepted OUT-04 timeline の 1080x1920 internal candidate、OUT-06 は accepted OUT-05 opening と authoritative cut_003 の complete narrative internal delivery candidate を生成する。`serve_review` は固定rootの127.0.0.1 byte-range localhost review serverであり、atomic staging、ASS/libass、reframe/audio/provenance readback を持つが、GUI action / production render / publishing / production subtitle design acceptance は行わない |
...
| `src/integrations/render/` | OUT-01 tiny diagnostic render、OUT-04 bounded editorial representative sequence、OUT-05 accepted-timeline vertical internal candidate、OUT-06 complete narrative internal delivery candidate。FFmpeg/FFprobe で source video/audio + validated cut ranges を diagnostic/review output artifact にし、subtitle timing・sequence rebasing・reframe/audio/probe readback を保持する | URL / VOD 取得、STT、cut 候補抽出、production subtitle design、GUI action、production render acceptance |
