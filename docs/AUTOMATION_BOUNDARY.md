...
| Local/External tool | diagnostic render proofs / editorial and vertical candidates / operator delivery pack | `src/cli/render_tiny_proof.py` / `src/cli/build_editorial_sequence.py` / `src/cli/build_vertical_short_candidate.py` / `src/cli/build_complete_narrative_short.py` / `src/cli/build_real_unused_range_short_minibatch.py` / `src/cli/build_operator_delivery_pack.py` / `src/cli/reconstitute_out07_review.py` / `src/cli/build_out07_direction_proxy.py` / `src/cli/serve_review.py` / `src/integrations/render/` | OUT-01 は tiny diagnostic proof、OUT-04 は explicit ordered 2-3 cut representative sequence、OUT-05 は accepted OUT-04 timeline の 1080x1920 internal candidate、OUT-06 は accepted OUT-05 opening と authoritative cut_003 の complete narrative internal delivery candidate。OUT-07 strict exact route は受理済み baseline SHA／size／duration を入力 gate にし、MP4 を byte-copy した上で、その動画内 frame＋既存 burn-in 字幕だけの cover を生成する。別系統の `build-out07-direction-proxy` は exact baseline を入力せず、既知 source revision、caption authority、scene/cue/timing mappingを検証して単一 cover 方向 proxy と list／UI／4:5／mapped-source evidenceだけを生成する。semantic proxy は accepted baseline、byte/pixel equivalence、canonical cover bytes、baseline video availability を主張せず、baseline video や旧候補も page に含めない。OUT-08 は declarative plan と episode authority を照合し、既使用 cut_001-003 と reject の cut_009 を除外した未使用範囲から、相互非重複の 1-2 本を 1080x1920 internal review candidate として atomic package に生成する。navigation frame は導線専用で thumbnail ではなく、review page の人間判断を acceptance に代用しない。`serve_review` は固定 root の 127.0.0.1 byte-range localhost review serverであり、GUI action / production render / publishing / upload / thumbnail upload / visibility / made-for-kids / production subtitle design acceptance は行わない |
...
| `src/integrations/render/` | OUT-01 tiny diagnostic render、OUT-04 bounded editorial representative sequence、OUT-05 accepted-timeline vertical internal candidate、OUT-06 complete narrative internal delivery candidate、OUT-07 accepted-baseline native-cover operator package、OUT-08 authoritative unused-range vertical minibatch。OUT-07は固定SHAのbaseline byte copy、動画frame抽出、一覧/4:5 preview、unchanged metadata、acceptance/gate/provenance/determinism/manifest readbackだけを担当する。OUT-08は declarative plan、source/caption/edit/decision/boundary/ledger/rights authority の照合、未使用範囲の timeline/subtitle rebase、1-2本の atomic package、media/audio/boundary/navigation/manifest readbackだけを担当する | 受理済みbaselineのrender/remux/transcode/edit/retime、URL / VOD / reference pixel取得、STT、自動cut候補採択、production subtitle design、GUI action、production render acceptance、upload、thumbnail upload、visibility、made-for-kids、publishing |
