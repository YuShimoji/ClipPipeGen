...
| Local/External tool | OUT-13 caption-evidence editorial representative video | `src/cli/build_editorial_video_candidate.py` / `src/integrations/render/editorial_video_candidate.py` / `src/cli/serve_review.py` | 取得済み実sourceとhash-bound editorial plan、transcript、official sidecar caption、rights snapshotから、非連続・時系列保持のselected/omitted ranges、根拠segment、context、字幕presentation、H.264/AAC render、full validation、manifest、video-first localhost reviewをatomic packageにする。source-native対話captionのselected-range非存在を証拠化し、speaker未確認時はbadgeを描画しない。rights、human acceptance、production subtitle/design/render acceptance、thumbnail、public/publishing/uploadは推定・実行しない |
