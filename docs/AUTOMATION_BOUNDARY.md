...
| Local | episode review bundle（diagnostic / representative review entry） | `src/cli/build_episode_review_bundle.py` / `src/pipeline/episode_review_bundle.py` | SH-08 実装済み。既存 episode / review artifacts を `review_manifest.json` と `index.html` に束ね、playable MP4 または contact sheet を HTML report より先に提示する。render / fetch / upload / rights approval / production acceptance は行わない |
