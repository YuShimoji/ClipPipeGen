...
- INT-02c の `local-media-audio` はローカル file の FFmpeg normalize のみ実装済み。INT-02e の `yt-dlp-audio` source audio URL fetch は actual smoke まで完了。`fetch-source-video` / GUI fetch button は未実装。
