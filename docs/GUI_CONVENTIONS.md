...
- ローカルだが高影響の操作（`set-compliance --status passed`、`register-material`、`patch-thumbnail`）は **確認 dialog を必須**とする（Phase 2 / SH-03b）。dialog には次の3要素を出す:
  - **command 文字列**（`python -m src.cli.main <subcommand>` と渡す引数の要約）
  - **summary**（ローカル CLI かどうか、network call の有無）
  - **reason**（なぜ確認させるか — gate 意義、bypass 不可性、INVARIANTS との接続）
