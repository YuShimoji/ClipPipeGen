...
`patch-thumbnail` で slot 差し替え可能な base template `.ymmp` を YMM4 で authoring する手順。現時点では GUI/CLI に template authoring 機能がないため、本ドキュメントは YMM4 での実務手順をまとめる。
...
> **注意**: `samples/` 配下に置く場合は `.gitignore` の対象範囲を確認。雛形をリポジトリにコミットしたい場合は、実ファイルを含めるかプレースホルダーで構成するかをそのスライスの目的に合わせて決める。
...
## 本ガイドの対象外（別 feature / 別工程で扱う）

- 構図・配色・最終クリック感の決定 — YMM4 上の人手作業
- 透過PNG 生成 — 外部前処理（bg-removal）または `INT-04` で扱う
- ymmp のゼロ生成 — 未実装。必要になったら別 feature として起票
- サムネテンプレを Python から authoring — 未実装。本ツールは slot patch を担当
