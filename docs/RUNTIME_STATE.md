...
- **current_slice**: Slice 2 — INT-02h `fetch-source-video --mode yt-dlp-video` is implemented。`src/integrations/asset_fetch/yt_dlp_video.py` adapter は yt-dlp path discovery、URL scrub、dry-run safe command plan、actual fetch、chosen format readback、許容 container ホワイトリスト（mp4 / mkv / webm）、FFprobe metadata readback、partial download cleanup、source_video.* 既存検出を担う。CLI は `--mode yt-dlp-video` / `--source-url` / `--format-selector`（default `best[ext=mp4]/best[ext=mkv]/best[ext=webm]/best`） / `--yt-dlp-path` を受け、`source_video.<ext>` / `sidecar.json` / `fetch_receipt.json` / `material_ledger.json` entry を生成する。rights snapshot は `hard_gate=false` / `production_acceptance=false`、`source_pipeline.intermediate_retained=false`。fake runner / dry-run / monkeypatch test で `uvx pytest -q`（173 passed） + GUI smoke 全 pass。実 URL operator smoke は user-supplied URL を待つ
- **next_action（assistant 側）**: INT-02h 実装と CI は完了。次は operator から URL を 1 本受け取り、ignored episode（例: `episodes/int02h_operator_smoke_<date>`）で `--dry-run` 後 actual run を行い、`source_video.<ext>` / `sidecar.json` / `fetch_receipt.json` / `material_ledger.json` entry と FFprobe metadata、URL scrub、chosen_format、`audit-material-ledger --format json` `ok=true` を readback する。smoke 後は INT-02h closeout commit を更新し、Phase 0 一本通し（実 URL → rendered_video.mp4 + CSV cut list）に進む。日本語 STT 破綻は Phase 0 で観測根拠として受容し、Phase 1 候補（vosk-model-ja / whisper.cpp 等の operator setup）に回す
...
- Operator smoke pending — actual URL smoke は operator が URL を 1 本選定するまで待つ。INT-02e と同じ手順で fresh ignored episode（例: `episodes/int02h_operator_smoke_<date>`）に対して dry-run → actual run → `source_video.<ext>` / sidecar / receipt / ledger / FFprobe metadata / URL scrub / chosen_format / `audit-material-ledger --format json` `ok=true` を readback し、結果を後続 commit で記載
