...
health: source_fetch_prep_ready_package_ready
progress_pct: 100
last_touched: 2026-07-08
next_review_due: explicit_private_fetch_smoke_approval_required
active_artifact: clip-ews05-human-ok-fetch-prep-ready-package-v0-001
...
latest_human_ok_fetch_prep_ready_commit: branch_head
...
Checkpoint date: 2026-07-08 JST. A new terminal should fetch the remote and
check out the active EWS human OK fetch-prep package branch before reading
project context:

```powershell
git fetch origin
git switch codex/ews-05-human-ok-fetch-prep-ready-v0
git pull --ff-only
```
...
The latest tracked slice is EWS-05. It packages the human operator's source
identity OK report for the current CPD-12 source-backed candidate and proves,
through EWS-03 and EWS-04 in a tempdir smoke, that the local plan reaches
`prep_state=ready_for_future_private_fetch_plan`. The human report says the URL
opens, the candidate intent is consistent, and the content matches the
explanation. The worker did not open the URL.

EWS-05 does not fetch media, create real tracked `episodes/` content, generate
transcript/render/thumbnail media, approve rights, mark production/public ready,
or publish. It records only tracked local readback JSON/Markdown and tempdir
smoke evidence. The repeatable commands are:
