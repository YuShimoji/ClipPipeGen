...
Python may generate local diagnostic/internal render artifacts only inside the render integration boundary (`src/integrations/render/`, `render-tiny-proof`, the bounded `build-editorial-sequence` route, the accepted-timeline `build-vertical-short-candidate` route, the bounded `build-complete-narrative-short` route, the bounded `build-real-unused-range-short-minibatch` route, and OUT-12 `build-real-video`). Such artifacts must keep production/public gates closed, must write receipt / manifest / validation / review readback, and must not be described as production render acceptance, public-ready render, production subtitle burn-in/design acceptance, rights approval, or creative acceptance.
...
- **Editing core は transcript／cut EDL／字幕案／配置データまで**。OUT-12のTimeline IR選択・cut/concat/H.264/AAC render・validationは`src/integrations/render/`内のinternal automation routeであり、coreへFFmpegを漏らさない。OUT-01以降のdiagnostic/internal outputと同様、生成MP4の存在はproduction subtitle/design/render acceptance、rights、creative/public/publishing acceptanceを意味しない。
- **OUT-07 の strict exact route と semantic direction proxy route を混同しない**。strict route は accepted baseline の SHA／size／duration、byte-copy、acceptance inheritance gate を必須とする。proxy route は同一 provider identity／scene／cue／timing／caption authority による cover 方向確認に限定し、accepted baseline、byte equivalence、pixel equivalence、canonical accepted cover bytes、baseline video availability を主張しない。
...
- `src/integrations/render/` — diagnostic/internal output adapter。OUT-01〜11のbounded proof/candidate/review経路に加え、OUT-12は取得済み実sourceからanalysis、chronological Timeline IR、caption timing remap、H.264/AAC長尺render、full validation、manifest、localhost review、fingerprint resumeを一コマンドで生成する。URL取得、rights判断、production subtitle/design/render acceptance、winner、thumbnail、public/publishing/uploadは含まない。
