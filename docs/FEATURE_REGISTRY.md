...
| INT-02h | yt-dlp-video source video URL fetch | proposed | INT-02g contract に従い、`fetch-source-video --mode yt-dlp-video` を source video URL fetch のみに限定して実装する後続 slice。`src/integrations/asset_fetch/yt_dlp_video.py` adapter、yt-dlp path discovery、dry-run command plan、URL scrub、許容 container ホワイトリスト、chosen format readback、FFprobe metadata、sidecar / receipt / ledger / rollback、fake runner / dry-run tests、operator による実 URL smoke までを範囲とする。FFmpeg normalize、render / encode、cut / concat、subtitle burn-in、GUI fetch button、`transcribe-audio` URL fetch、`render-tiny-proof` URL fetch、publishing は含めない |
