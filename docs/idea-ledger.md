...
| G3: exact artifact access/recovery | humanがOUT-13 exact bytesを探索なしで見られるようにする | 生成host access、承認済みtransport、またはhash-bound plan復元 | root checkoutはplan/package不在 | Human owner: transport/rebuild route選択。Agent: identityを混同せず実行 |
| G4: internal editorial acceptance | composition、subtitle presentation、picture/audioを一本で判断 | G3、exact SHA、single review entry | pending | Human reviewer: accept / bounded repair / rejectをSHAへbind |
...
- **Review**: exact OUT-13 packageへ到達できるhostで一本をhuman reviewする。現在の最短critical path。
- **Restore**: packageが利用できない場合、private transportかhash-bound plan復元のどちらかでreview surfaceを再構成する。
- **Audit**: media accessを待つ間に、plan/failure/resume contractを監査し、修復時のstop条件を先に明文化する。
