...
Post-spec implementation note: INT-02e はこの contract に従い `fetch-source-audio --mode yt-dlp-audio` を source audio URL fetch のみに限定して assistant-side 実装 in_progress。実 URL operator smoke は user-owned URL selection and rights / terms review 待ち。
