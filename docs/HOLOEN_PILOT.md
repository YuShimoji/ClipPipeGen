...
HoloEN pilot に使う URL は、以下を満たすものを operator が選定する。assistant は勝手に選定しない。
...
## Runbook（operator-supplied URL があるとき）
...
$URL="<operator-supplied HoloEN public VOD URL>"
...
1. operator-supplied HoloEN public VOD URL が **避けるべき素材条件** を満たさないと operator が判定済み
...
## Blocked state — URL 未指定の場合

operator が URL を提供しない / 提供できない時は、本スライスを **`in_progress: blocked_waiting_for_url`** として保持する。assistant は勝手に任意 URL を選定しない。

blocked closeout で残すべきもの:

- `docs/HOLOEN_PILOT.md`（本文書）
- `docs/FEATURE_REGISTRY.md` の `HoloEN-01` 状態行と遷移ログ
- `docs/RUNTIME_STATE.md` Phase 0.5 セクション
- `docs/HANDOFF.md` の Sync Point / Recommended Next Slice
- placeholder URL での dry-run readback（no-network、CLI surface 確認）

operator が後で URL を指定したときの再開手順は **Runbook** セクションの通り。
