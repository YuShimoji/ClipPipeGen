...
1. 仮の `rights_manifest` を作成し、人手で `compliance_check.status` を `passed` にする
...
## スコープ外（Slice 1 では触らない）

- 元動画ダウンロード（次スライス。`src/integrations/asset_fetch/` は空のまま）
- 背景切り抜き API 呼び出し（外部処理結果＝透過PNG を**手動で受け取る**形のみ）
- 動画 cut detection・字幕生成（次スライス以降）
- YouTube upload（後続スライス）
- GUI（CLI のみで完結。後続スライスで GUI 追加検討）
...
│   │   ├── compliance_gate.py     # 全 CLI が import する gate enforcement
...
人手で確認した結果を記録。`status=passed` にする際、CR-01 の auto-fail 条件（VOD not public、permitted=false の third_party_ip 等）に該当しないことを CLI が再検証し、該当すれば失敗する。
...
2. material validation（sidecar restrictions / source.kind / license.kind）
...
- Slice 1 の目的は schema・gate・bridge の運用感確認。GUI は設計コストが大きい
...
- [ ] `patch-thumbnail` が `thumbnail_use=denied` 素材で fail する
...
## 危険な自動化境界（Slice 1 で起きうる事故）

| 操作 | 危険 | 対策 |
|---|---|---|
| `set-compliance --status passed` を CLI フラグで自動化 | 権利確認を skip して passed にしてしまう | `--checked-by` 必須、`--checked-by` が ユーザー名で始まらないと失敗。auto-fail 条件は CLI 側で再検証 |
...
| sidecar 必須の例外 flag 追加（`--skip-sidecar`） | 出典不明素材が混入 | このような flag を追加しない。INVARIANTS の永続条件 |
