...
last-verified URL は `http://127.0.0.1:8071/index.html`。PID `39652` が exact
...
| `candidate_02` | `cut_006` tail `81.298–98.315`; `cut_007` `98.315–116.467`; `cut_008` `116.934–135.219`; dependent payoff `137.054–138.055` | `54.455s` / `54.5s` | 55 | `c6d3ff01624c359c1ca0030bb360eb3906e6b35a62ec439c0efb14378e9c8320` | 遭遇と対立の進展から最後の短い勝敗 payoff までが一本の流れとして持続するかを見る。 |

candidate 01 は `9.409s` に hard cut を持つ。candidate 02 は `17.017s`、
`35.169s`、`53.454s` に hard cut を持つ。`cut_009` の final cut decision は
`reject` のままで、authority は変更していない。最後の 1.001 秒は
`sub_102` の dependent payoff だけを使い、`cut_009` 自体の promotion や
reversal を意味しない。
...
self-integrity `219e709f4f88e69e09893a782a761bfd51f84ffdb09eea7b0428af78733a102d`
...
`-14.21 LUFS / -1.49 dBTP`。いずれも package readback に input/output
loudness、true peak、normalization decision を記録した。

bounded subtitle repair 前の localhost browser check では両 video が
`readyState=4`、media error `null`。desktop/mobile viewport とも horizontal
overflow なし、console warning/error なしを確認した。最終 repair package は
HTTP `200`、byte range `206 Partial Content`、ffprobe、full decode を再確認した。
