...
日本語 Vosk model は pipeline 接続としては機能したが、実コンテンツの字幕品質にはまだ届かない。transcript の例は「話者 も 欲しい ねぇ じゃあ を よ ねー 何 と もう 一 台 です かー...」のように、単語の拾い方はあるものの、人間がそのまま字幕・cut 判断に使える品質ではない。したがって、次に単純な render polish へ進むより、transcript review / correction を先に用意した方が workflow 全体の摩擦が下がる。
...
|---|---|---|---|
| STT 品質・補正 | JP public VOD で 26 segments 生成、ただし字幕品質は未受容 | 実装接続は進んだが、制作判断の入口で停滞 | `ED-09` transcript review / correction workflow を起票し、Vosk 出力を人間が直せる artifact にする |
| cut review | 6 candidates のうち 3 needs_review | 自動候補は出るが、境界確認が人手依存 | `ED-09` と合わせて修正済み transcript / selected cut を再 export できるようにする |
| production render / subtitle design | diagnostic render は通るが、本番字幕設計・safe-area・typography は未実装 | transcript/cut が未受容のため先に進むと手戻りが大きい | transcript review 後に production subtitle/render acceptance を切る |
...
| SH-02 episode_pack | proposed | artifact が増えたため価値は上がったが、publish_draft がまだ無い | transcript review または production render 後にまとめる |
...
|---|---|---|
| Advance: `ED-09` transcript review | STT 出力を制作判断に使える形へ直せる | corrected transcript から cuts / subtitles / NLE / render を再生成できる |
