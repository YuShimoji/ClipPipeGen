...
|---|---|---|
| Episode | episode の現在状態を一覧する | `rights_manifest` / `material_ledger` / `edit_pack` / `thumbnail_patch_input` / `thumbnail_patch_result` の存在、gate 状態、次に必要な操作 |
...
| Editing | edit_pack の状態と手動 cut 入力（ED-01 / ED-02a の範囲のみ） | `init-edit-pack` / `add-cut-candidate` / `validate-edit-pack` 実行導線、cut_candidates 件数、selected 件数、schema 違反 |
...
Editing タブは **ED-01 / ED-02a の手動 cut 入力範囲だけ** GUI に出す。自動 cut detection（ED-02 本体）／ 文脈チェック（ED-03）／ 字幕生成（ED-04）／ NLE export（ED-06）は実装されるまで GUI 上に表示しない（操作可能に見えると gate 境界が濁る、GUI_CONVENTIONS §5）。

Publishing は引き続きタブとして置かない。FEATURE_REGISTRY の `PB-*` / `INT-01` が実装される段階で再評価する。
