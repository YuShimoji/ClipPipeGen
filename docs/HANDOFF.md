...
- Latest local evidence slice: scoped subtitle-overlay visual proof for explicit target cuts `cut_002` / `cut_003`. Local ignored `subtitle_overlay_visual_proof_report.*`, `subtitle_overlay_visual_proof_cut_002.mp4/png/srt`, `subtitle_overlay_visual_proof_cut_003.mp4/png/srt`, and refreshed `representative_visual_proof_report.*` exist under the R3 review directory. The report records `jp_clip_readable_v1` as a diagnostic style direction preset, separates qualitative subtitle intent from unpinned FFmpeg/libass style parameters, embeds or directly links proof PNG/MP4 artifacts, and keeps visual inspection diagnostic-only. This proof run is stale for the later boundary-adjusted `cut_003`: the current authority is `22.606 -> 49.566` with `sub_010..sub_029`, while the proof report/SRT still read `22.606 -> 41.725` with `sub_010..sub_024`.
- Latest local proxy decision handoff: ignored `cut_002_cut_003_text_proxy_review.json` / `.html`, `cut_002_cut_003_operator_proxy_decision_handoff.json` / `.html`, and scoped `chapter_revision_patch.cut_002_cut_003_proxy.template.json` / `.csv` exist. Source media is available from `material_ledger.json` material paths. The regenerated handoff reads `visual_proof_status=available_requires_human_review`; `proxy_decision` now includes `proceed_with_limitations` for candidate routing with visible limitations/watch items; templates remain blank/default, while the filled operator patch is the current decision authority; `cut_003` retained context risk remains active.
...
- Latest Review Contract Taxonomy audit: [CUT_003_REVIEW_CONTRACT_TAXONOMY_AUDIT.md](CUT_003_REVIEW_CONTRACT_TAXONOMY_AUDIT.md) preserves the current audit. The proof-level taxonomy is blocked because the current `edit_pack.json` authority is `cut_003=22.606 -> 49.566` with `sub_010..sub_029`, but `subtitle_overlay_visual_proof_report.json` and `subtitle_overlay_visual_proof_cut_003.srt` still read `22.606 -> 41.725` with `sub_010..sub_024`. `style_direction_preset=jp_clip_readable_v1`, `production_candidate=false`, `rights_status=pending`, and `production_usage_allowed=false` remain visible, but positive flags cannot compensate for missing proof coverage.
- Current recommended decision: first regenerate only the ignored `cut_003` subtitle-overlay proof/report/SRT from current authority, keeping `sub_030` excluded and all rights/production flags unchanged. Then rerun the narrow taxonomy audit before asking for final cut_003 visual/proxy decision input. If `cut_review_packet.json` / `cut_decision_packet.json` `candidate_reason` prose will be human-facing, run a narrow review/decision reason readback cleanup after that. A fresh checkout or workspace missing ignored artifacts may still report `review_blocked_missing_artifacts`; either state remains diagnostic-only and not production, creative, publishing, or rights acceptance.
...
- Current pushed resume point: this tracked handoff/runtime refresh after the cut_003 Review Contract Taxonomy audit correction found stale subtitle-overlay proof evidence. Confirm the exact hash with
  `git log -1 --oneline --decorate` after pulling.
- Verified base before this refresh: origin/main parity `0 0` in this workspace before this handoff refresh.
...
- Latest taxonomy-audit validation: 2026-06-08 JST; parsed `edit_pack.json`, `subtitle_overlay_visual_proof_report.json`, `representative_visual_proof_report.json`, `cut_review_packet.json`, `cut_decision_packet.json`, `chapter_revision_board.json`, `cut_002_cut_003_operator_proxy_decision_handoff.json`, and `subtitle_overlay_visual_proof_cut_003.srt`. Added ignored `review_contract_taxonomy_audit_cut_003.json` / `.html` and tracked this context in `docs/CUT_003_REVIEW_CONTRACT_TAXONOMY_AUDIT.md`. `status-episode` reports `operator_review.review_ready=true`, but the cut_003 proof-level taxonomy remains blocked until the stale proof media/report are regenerated for `22.606 -> 49.566`. Proof media was not regenerated. Tracked code was not changed; tracked edits are docs-only.
...
| `cut_003` | text/timing authority is current at `22.606 -> 49.566` with 20 subtitles, but the currently present subtitle-overlay proof media/report still read old `22.606 -> 41.725` | do not ask for final visual/proxy decision from the stale proof; regenerate the proof or explicitly waive first | retained context risk remains visible; no production subtitle/design/render acceptance |
...
export have now been regenerated from this ignored local edit pack. Treat
proof/render artifacts as stale_reference until regenerated, and treat stale
`candidate_reason` prose in `cut_review_packet.json` /
`cut_decision_packet.json` as a remaining narrow cleanup item if those fields
will be human-facing.

After the 2026-06-08 taxonomy audit, treat the current
`subtitle_overlay_visual_proof_report.json`,
`subtitle_overlay_visual_proof_cut_003.srt`, and linked proof PNG/MP4 as stale
for final cut_003 visual/proxy decision because they still cover only
`22.606 -> 41.725`. The chapter board and scoped proxy handoff timing/text
readbacks are current, but the nested visual-proof readback inherits that stale
proof artifact.
...
|:--:|---|---|---|:--:|
| 1 | Resolve or waive: missing `visual_proof_cut_001.png` | This current workspace reports `review_blocked_missing_artifacts` | Opens the global R3 review gate only after regeneration or explicit waiver | High |
| 2 | Review: scoped `cut_002` / `cut_003` overlay proof | The scoped subtitle-overlay proof and handoff exist locally | Human can inspect diagnostic overlay readability without claiming acceptance | High |
| 3 | Advance: `cut_002` / `cut_003` proxy decision | The scoped handoff keeps operator fields blank / `undecided` | Machine-readable routing into subtitle, boundary, render plan, or NLMYTGen handoff work after visual inspection | High |
