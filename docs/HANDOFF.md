...
- Latest Review Contract Taxonomy audit: [CUT_003_REVIEW_CONTRACT_TAXONOMY_AUDIT.md](CUT_003_REVIEW_CONTRACT_TAXONOMY_AUDIT.md) preserves the current audit. The cut_003 subtitle-overlay proof gates are evidence-backed for diagnostic human visual review at `22.606 -> 49.566` with `sub_010..sub_029`, `sub_025..sub_029` included, `seg_000030` / `sub_030` excluded, `style_direction_preset=jp_clip_readable_v1`, `production_candidate=false`, `rights_status=pending`, and `production_usage_allowed=false`. However, the locally present `chapter_revision_board.json` and `cut_002_cut_003_operator_proxy_decision_handoff.json` still read old cut_003 `22.606 -> 41.725`; do not use them as final proxy decision input until regenerated or explicitly waived.
- Current recommended decision: first regenerate or update only the ignored downstream operator handoff / chapter board / scoped patch template so cut_003 uses current authority `22.606 -> 49.566`. If `cut_review_packet.json` / `cut_decision_packet.json` `candidate_reason` prose will be human-facing, run a narrow review/decision reason readback cleanup after that. A fresh checkout or workspace missing ignored artifacts may still report `review_blocked_missing_artifacts`; either state remains diagnostic-only and not production, creative, publishing, or rights acceptance.
...
- Current pushed resume point: this tracked handoff/runtime refresh after the cut_003 Review Contract Taxonomy audit and downstream-handoff readiness correction. Confirm the exact hash with
...
- Latest taxonomy-audit validation: 2026-06-08 JST; parsed `subtitle_overlay_visual_proof_report.json`, `representative_visual_proof_report.json`, `cut_review_packet.json`, `cut_decision_packet.json`, `chapter_revision_board.json`, and `cut_002_cut_003_operator_proxy_decision_handoff.json`. Added ignored `review_contract_taxonomy_audit_cut_003.json` / `.html` and tracked this context in `docs/CUT_003_REVIEW_CONTRACT_TAXONOMY_AUDIT.md`. `visual_proof_cut_001.png` is currently missing, so global review is still blocked. Proof media was not regenerated. Tracked code was not changed. `git diff --check` passed and `git ls-files episodes` remained empty.
...
| `cut_003` | current proof authority is `22.606 -> 49.566` with 20 subtitles, but the currently present proxy/chapter downstream artifacts still read old `22.606 -> 41.725` | do not ask for final proxy decision from the stale handoff; regenerate or explicitly waive first | retained context risk remains visible; no production subtitle/design/render acceptance |
...
After the 2026-06-08 taxonomy audit, also treat the current
`chapter_revision_board.json` and
`cut_002_cut_003_operator_proxy_decision_handoff.json` as stale for final
cut_003 proxy decision because their cut_003 entries still read
`22.606 -> 41.725`. The proof report itself can still be inspected for
diagnostic human visual review.
