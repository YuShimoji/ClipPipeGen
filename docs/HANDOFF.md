...
|:--:|---|---|---|:--:|
| 1 | Official subtitle track import / transcript alignment | R2 で 37 公式字幕 event が Vosk segment 外に残った | new CLI/schema slice。権利 readback は pending 維持 | ★★★★★ |
| 2 | whisper.cpp / OpenAI Whisper 採用比較 | 公式 subtitle がない素材では Vosk EN/JP の誤認識と segmentation gap が workflow bottleneck | 依存追加または API 契約判断 | ★★★★ |
| 3 | production subtitle/render acceptance | R2 の narrowed cuts は 5 passed まで来たが caption completeness / typography / safe-area / full render policy が未定 | caption gap 方針と production subtitle design の定義 | ★★★ |
| 4 | JP-Pilot final cut policy | R2 cuts は 10.86s〜23.13s で扱いやすいが creative selection は未受容 | official caption import 後に採用 cut を絞る | ★★★ |
...
`chosen_format` の archive.org extractor 限界、git-bash の electron smoke ドリフト、INT-02 umbrella status の drift は small Audit 候補として保留。JP-Pilot-01R2 後に残る最大停滞は、Vosk segment ベースの patch だけでは公式字幕 event の欠落を回収できないこと。
