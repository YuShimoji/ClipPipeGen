...
- Latest completed diagnostic slice: `JP-Pilot-01R` ED-09 corrected rerun. It applied a 7-segment transcript review patch to the ignored JP-Pilot episode, regenerated cuts/context/subtitles/NLE/render, and made the JP-Pilot rights manifest schema-readable while keeping `compliance_check.status=pending`.
- Current recommended decision: choose between JP-Pilot review coverage + selected cut narrowing, official subtitle track import / transcript alignment, or STT provider comparison. Do not jump straight to production render while 19/26 transcript segments remain unreviewed.
...
- Sync commit: JP-Pilot-01R corrected rerun closeout on `main` (see latest `git log --oneline -1` after pull)
- Latest implementation slice before JP-Pilot-01R: ED-09 done。`review-transcript` CLI と pipeline patch 適用、`status-episode` review readback、`export-nle` warning 更新、docs registry 更新を含む
...
- Previous recommendation resolved by JP-Pilot-01R: ED-09 correction entry works on real JP material, but partial correction is not enough for production acceptance. The remaining question is whether to expand review coverage, import official subtitle tracks, or compare STT providers for sources without official captions.
- Latest completed feature-slice closeout before this handoff note: ED-07c language/model validation closeout
- Latest local verification: 2026-05-26 JST JP-Pilot-01R closeout; `uvx pytest -q` (188 passed), `npm run smoke` OK, `npm run smoke:electron` OK, `git diff --check` clean.
...
- The transcript is technically real but not creatively acceptable as-is. ED-09 added the review / correction entry point, and JP-Pilot-01R proved a 7-segment corrected rerun can flow back into cuts / subtitles / NLE / render. The next bottleneck is review coverage and selected cut narrowing, not render polish or publishing.
...
| Transcript | `transcribe-audio --engine fake` and optional `--engine vosk --model <path>` exist; ED-09 review patches can mark corrected transcript text; JP-Pilot-01R has 7 accepted / 19 unreviewed segments | Review coverage remains the main bottleneck; partial correction is not production transcript acceptance |
| Edit pack | `transcript.json` can feed cut candidates, context checks, subtitles, and ED-06 CSV export; JP-Pilot-01R produced 5 cuts, 26 subtitle drafts, and 2 needs_review context checks | selected cuts are still broad and include unreviewed transcript segments, so cut narrowing should come before production edit acceptance |
...
JP-Pilot-01R で補正済み transcript を downstream に戻せることは分かった。次は「補正 coverage を増やす」「公式 subtitle track を import 可能にする」「公式字幕がない素材のために STT provider を比較する」のどれで transcript bottleneck を下げるかを選ぶ段階。

| # | 候補 | 観測根拠 | unblock 条件 | 推奨度 |
|:--:|---|---|---|:--:|
| 1 | JP-Pilot review coverage + cut narrowing | 7 accepted だけでは selected cuts に未補正 transcript が混ざる | 追加 review patch と selected cut policy | ★★★★★ |
| 2 | Official subtitle track import / transcript alignment | JP-Pilot-01R では公式 Japanese subtitles が最も有効な補正 source だった | new CLI/schema slice。権利 readback は pending 維持 | ★★★★★ |
| 3 | whisper.cpp / OpenAI Whisper 採用比較 | 公式 subtitle がない素材では Vosk EN/JP の誤認識が workflow bottleneck | 依存追加または API 契約判断 | ★★★★ |
| 4 | production subtitle/render acceptance | diagnostic burn-in はあるが typography / safe-area / full render policy が未定 | review coverage と selected cuts の受容性確認 | ★★★ |
| 5 | GUI fetch/render/export actions | CLI 経路は通るが operator が各 CLI を手動で連結する摩擦が残る | core artifact contract 維持、確認 dialog / receipt readback | ★★★ |
| 6 | `TH-01 walkthrough closeout` | サムネレーン user-owned acceptance 未踏 | 実 YMM4 .ymmp + `config/nlmytgen_path.json`（operator-owned） | ★★ |
| 7 | Publishing (INT-01 + PB-01..04) | 縦糸の出口接続 | YouTube OAuth credentials と production acceptance | ★★ |

`chosen_format` の archive.org extractor 限界、git-bash の electron smoke ドリフト、INT-02 umbrella status の drift は small Audit 候補として保留。JP-Pilot-01R 後に残る最大停滞は、partial review では production-adjacent cut/subtitle acceptance に届かないこと。
...
After `JP-Pilot-01R`, the project should deliberately move toward one of these production-adjacent bottlenecks:

| Candidate | Usefulness | Why it matters | Risk |
|---|---:|---|---|
| review coverage + cut narrowing | 9/10 | JP-Pilot-01R improved counts but left 19/26 segments unreviewed and selected cuts broad | Keep transcript approval separate from edit/render/publish acceptance |
| official subtitle import | 9/10 | The official Japanese subtitle track provided a reliable correction source for this public VOD | Must keep rights/readback pending and avoid production claims |
...
| production subtitle/render acceptance | 6/10 | Diagnostic burn-in works, but typography/safe-area/full render policy is absent | Premature until reviewed cuts are narrow enough |
...
Recommended continuation after `JP-Pilot-01R`: prefer official subtitle import / transcript alignment if the goal is to reduce correction cost on captioned public VODs; prefer STT provider comparison if the next target lacks official captions; prefer review coverage + cut narrowing if staying on this JP-Pilot artifact. Do not count docs-only policy expansion, read-only GUI panels, or audit log polish as production progress unless they connect a corrected transcript, selected cuts, NLE export, or rendered video artifact.
