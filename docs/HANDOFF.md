...
- Current pushed resume point: this tracked proxy-decision allowed-value refresh
  after the 2026-06-07 JST schema/readback validation. Confirm the exact hash with
  `git log -1 --oneline --decorate` after pulling.
- Verified base before this refresh: `a06078e feat: add diagnostic subtitle style readback`
...
|---|---|---|---|
| `cut_002` | context `passed`, compact 4.838s setup beat, 2 subtitles, line wrap proxy review required | `proxy_decision=undecided`, intent/script/subtitle request blank; `proceed_with_limitations` is available if the operator accepts readability while keeping the long-line watch visible | subtitle-overlay typography, safe-area, and timing sync |
