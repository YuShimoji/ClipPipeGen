...
INT-02 系の素材取得コマンドが、実行内容・生成物・rollback 対象を記録する receipt。`fetch-source-audio` / `fetch-source-video` は `materials/<material_id>/fetch_receipt.json` を作る。
...
| `mode` | string | `fake` / `local-media-audio` / `yt-dlp-audio` / `local-media-video` / future mode |
| `source_url` | string/null | URL 取得 mode の scrub 済み readback。query / fragment / userinfo は残さない。`local-media-audio` では `null` |
...
| `input.source_url` | string/null | URL 取得 mode の scrub 済み入力 URL readback |
| `input.local_path` | string/null | local media mode の入力 file |
| `outputs[]` | array | 生成 file の path / hash / byte size / duration readback |
| `video_metadata` | object | source video mode の FFprobe readback。duration / codec / resolution / fps / container / stream count |
...
INT-02d は spec only。INT-02e では [YTDLP_AUDIO_SPEC.md](../../YTDLP_AUDIO_SPEC.md) に従い、`fetch-source-audio --mode yt-dlp-audio` を source audio URL fetch のみに限定して実装し、technical smoke URL の actual run で receipt / sidecar / ledger / WAV readback を確認済み。

| 項目 | Contract |
|---|---|
| mode | `yt-dlp-audio` |
| input | `input.source_url` には scrub 済み URL readback を残し、`input.local_path` は downloaded intermediate ではなく operator input なしを示す |
...
## INT-02f local-media-video 標準

`fetch-source-video --mode local-media-video` は、既存ローカル video file だけを入力にし、episode material directory へコピーして FFprobe metadata を readback する。render / encode / cut / concat / subtitle burn-in は行わない。

| 項目 | 値 |
|---|---|
| 入力 | `--source-path <path>`（`--local-media` alias あり）。URL / VOD / network fetch は拒否 |
| 出力 | `materials/<material_id>/source_video.<ext>`。入力拡張子を維持 |
| ledger | `kind="source_video"` / `subkind="source_video_original"` / `intended_uses=["editing_video"]` |
| FFprobe discovery | `--ffprobe-path` → `CLIPPIPE_FFPROBE` → PATH |
| metadata | duration、container、video/audio codec、resolution、fps/frame_rate、stream count |
| provider | `local-media` |
| rights | rights status は snapshot のみ。hard gate にしない |
| warning | source video acquisition は production / creative / publish acceptance ではない |
| partial failure | partial `source_video.<ext>` は削除。sidecar / receipt / ledger は書かない |
