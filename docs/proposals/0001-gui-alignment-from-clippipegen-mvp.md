...
|---|---|
| `ready` | 次工程に進める |
| `blocked` | schema / audit / gate 違反でブロック |
| `manual_needed` | 人間判断（compliance gate 等）が要る |
...
ClipPipeGen 側 schema: `thumbnail_patch_result` の `compliance_gate / material_validation / audit_result / patch_result.applied_slots[]` をそのまま GUI で 4 セクションに分けている。
...
ClipPipeGen の GUI MVP では、永続手動 gate（自動公開、外部 API、元動画ダウンロード、bg-removal 送信）に **disabled button すら置いていない**。誘惑させない方針。
...
| 危険操作 button 不在 | GUI 上の制作確認ボタンの有無方針 |
...
- スコープ: 4項目は粒度がバラバラ。lane state 語彙統一は最も小さく即効、危険操作 button 不在は思想方針なので合意までで足りる。多段 readback セクションは UI 改修コスト中、Settings readiness は ClipPipeGen 側にあるパターンの移植で済む。
